...
| 32 | 2026-04-19 | Title-tooltips on every icon button (NPS 7.0) | First-time visitor asked to "make the primary actions more explicit with text labels/tooltips near the icon buttons". Added `title` attributes to menu, refresh, header-settings, feedback FAB, and wx-search-btn so desktop users get hover tooltips and screen-reader users get an extra cue. No layout or mobile-behavior change — smallest possible slice at the discoverability/accessibility intersection. |
| 33 | 2026-04-19 | Contextual cross-links at bottom of Today tab (NPS 6.9) | Multiple testers asked for "contextual links between related views (Today → Forecast, Calendar date → History)" and flagged that the app feels linear/siloed. Added a two-button `.wx-context-links` row at the end of the Today pane: "See 7-day forecast →" switches to Forecast tab, "Open monthly calendar →" switches to Calendar tab. Plain cards with blue text so they read as links, not primary CTAs. Addresses the siloed-navigation complaint with a minimal, scannable nudge at the natural read-end of Today. |
| 34 | 2026-04-19 | One-line descriptions under each drawer nav item (NPS 8.0) | First-time tester asked to "make the drawer sections slightly more self-explanatory with one-line descriptions under each item". Added a `.nav-sub` 11px muted caption under each `.nav-main` label in the drawer: About ("What this app is and how it evolves"), Stats ("Aggregate NPS and feedback counts"), Progress ("NPS over time across AutoLoop cycles"), Settings ("Dark mode, large text, themes"), End ("Close this session"). Stacks inside the existing flex row with the icon, no layout overhaul. |
| 35 | 2026-04-19 | Roomier History table + larger recent-remove button (NPS 7.2) | Recurring feedback: "History observations table feels noticeably denser and tighter than surrounding panels" and "recent-city remove buttons are cramped". History row padding 8px→12px, font 12→13px, column widths widened 38→42px, and header weight 600→700 with darker color (#636366 vs #8e8e93) for scannability. Remove button: padding 6/10→8/14px, min-height 36px to hit Apple's 36px minimum for secondary controls. Row alignment gap added so values don't collide. |
| 36 | 2026-04-19 | Persistent "current section" breadcrumb in nav title (NPS 8.0) | Explorer tester asked for "a persistent breadcrumb or small 'current section' label when moving between deeper views". Cheapest possible implementation: switchTab() now updates the global nav-title from "Weather" to "Weather · Today" / "Weather · Forecast" / "Weather · Calendar" / "Weather · History" / "Weather · Search" as the user moves between tabs. Zero new DOM — re-uses the existing header slot — so no layout impact, and the breadcrumb is always visible regardless of scroll position. |
| 37 | 2026-04-19 | Contextual cross-links at bottom of History tab (NPS 8.0) | Same explorer feedback re-surfaced: "surface contextual links like 'View Forecast' / 'Open Stats' directly from Today and History". Cycle 33 added them to Today; this cycle extends the `.wx-context-links` row to the end of History: "Jump to calendar view →" and "Back to 7-day forecast →", re-using the same CSS and the same switchTab() event pattern. |
| 38 | 2026-04-19 | Large-Text mode now styles the weather screen + toast confirmation (NPS 6.8) | Root-cause fix for persistent "aging eyes, text too small" complaints and one grade-4 boomer. The `.app.large-text` class existed (cycle 15) but was only wired to the legacy news/article styles — toggling it did nothing on the weather screen, which is the primary content. Added a dedicated large-text block in weather.css that bumps every key weather surface: hero temperature 64→80px, hero city/phrase 16/18→20/22px, section labels 11→15px, forecast day/hi/lo, quick-stat values, tab labels, obs-table rows, recent/popular city chips, calendar cells, context links, history date fields. Added a "Large Text on / off" toast on toggle so the boomer actually sees the change register. Expected to shift aging-eyes grades up significantly. |
| 39 | 2026-04-19 | Inline "Keep typing" hint for single-character searches (NPS 7.0) | Security/edge-case tester: "the field did not visibly preserve the exact text I entered — unclear whether input was sanitized, rejected, or reformatted". Previously a 1-character query silently reverted the results panel to Recent/Popular defaults, which felt like silent rejection. Now the input handler shows an inline hint — `Keep typing — at least 2 letters to search for "<esc(q)>"` — so the user sees their value reflected (escaped) and understands exactly why no results are shown yet. The typed value itself in the input box is of course unchanged (it already was). |
| 40 | 2026-04-19 | Echo the typed query in "no letters detected" search message (NPS 7.0) | Same edge-case tester re-surfaced. Strengthened the invalid-input message so it echoes back exactly what the user typed (HTML-escaped): `No letters found in "<esc(q)>" — try a city name like London or Berlin`. Previously the message was generic and didn't confirm the user's input was received at all. Pairs with the keep-typing hint from cycle 39 to give consistent, explicit feedback across every search input state. |
| 41 | 2026-04-19 | Idle-prefetch Calendar & History data after Today loads (NPS 7.2) | Multiple "rage-click / impatient user" testers complained Calendar takes a moment to load. Added a requestIdleCallback (with setTimeout fallback) fired at the end of loadAll() that background-fetches `/api/weather/calendar`, `/api/weather/almanac`, and `/api/weather/history` into the in-memory caches. First-time tap into either tab now renders instantly from the already-populated cache instead of showing the "Loading calendar…" spinner. Zero visible change on the Today tab; pure perceived-performance improvement. Errors are silently swallowed so prefetch can never degrade the primary flow. |
| 42 | 2026-04-19 | Visible notice when control characters are stripped from search (NPS 8.0) | Edge-case tester: "leading null byte was sanitized away, which is fine, but no explicit validation message explaining the normalization". The search input handler now strips U+0000-U+001F and U+007F control characters explicitly (they were previously ignored by `trim()` but silently accepted by the underlying field), writes the cleaned value back into the input, and fires a `"Removed invisible characters"` toast so the user sees the sanitization happen. Hardens input handling and closes the loop on the "explain normalization" ask. |
| 43 | 2026-04-19 | Persistent "Tab · City" breadcrumb in nav title (NPS 6.9) | Multiple testers: "provide clearer breadcrumbs or a small 'current city' control near every view". Cycle 36 added the tab-name breadcrumb ("Weather · Today"). Extended it to include the current city: now reads `Today · Amsterdam`, `Forecast · London`, `History · Paris`, etc. updateNavTitle() is called from switchTab() and inherits the city update for free because selectLoc() already triggers switchTab('today') at the end. Zero new DOM. |
| 44 | 2026-04-19 | Server-side validation + rate limiting for /search and /history (NPS 7.0) | Security tester asked for "explicit server-side validation/sanitization for city search and date-range inputs" and "rate limiting and abuse detection on search/autocomplete". Added: (1) searchLimiter — 60 req/min — and historyLimiter — 30 req/min — in addition to the existing ask/feedback limiters; (2) /api/weather/search now caps the query at 64 chars, requires at least one letter, and short-circuits otherwise; (3) /api/weather/history now returns HTTP 400 with a neutral message if start/end fail a strict `YYYY-MM-DD` regex or if start > end. Defense-in-depth against probing and pathological input; no valid-use-case impact. |
| 45 | 2026-04-19 | Visible inline validation for History date-range (NPS 6.8) | Edge-case tester: "show clear inline validation for invalid or reversed date ranges". Previously clicking Go with a missing or reversed date silently did nothing (the `if(s&&e&&s<=e)` guard short-circuited without any feedback). Now the handler fires an explicit toast — "Pick a start and end date" when either field is empty, or "Start date must be before end date" when the order is wrong — matching the server-side 400 validation from cycle 44 so client and server tell the same story. |
| 46 | 2026-04-19 | More breathing room in Search recent/popular sections (NPS 6.8) | Persistent "Search feels cramped / dense" feedback across cycles 7, 35, 45. Bumped popular-city grid gap 8→10px, container padding 0/12/8→8/14/16px, per-chip padding 14/8→16/8px. Recent-item padding 16/14→18/16px, gap 8→10px, and added 8px bottom margin. Small but compounding effect — Search feels noticeably less packed without losing density. |
| 47 | 2026-04-19 | Prominent green toast variant for city-switch confirmation (NPS 7.0) | First-time visitor: "add a stronger success/selection confirmation after choosing a city". The standard dark toast was easy to miss. Added a `toast-prominent` CSS variant (bright green #22c55e, bold 16px, larger padding, shadow glow) and a duration option to `showToast(msg, opts)`. selectLoc() now calls it as `showToast('✓ Switched to 🇬🇧 London', { prominent: true, duration: 5000 })` — visibly different, longer on screen, includes the flag + checkmark so the confirmation is unmistakable. |
